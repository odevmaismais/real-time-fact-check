...
const isGarbage = (text: string): boolean => {
  ...
  // Single char safety:
  // Allow vowels/articles often used as starts: 'a', 'e', 'é', 'o', 'ó', 'u', 'à', 'y' (and numbers)
  // Reject random consonants like 'k', 'z', 'b' standing alone.
  if (t.length < 2 && !['a', 'é', 'e', 'ó', 'o', 'u', 'à', 'y', '1', '2', '3'].includes(t.toLowerCase())) return true;
  ...
  const hallucinations = [
    ...
    'a seguir', 'blá blá', 'etc etc',
    
    // Nonsense phrases common in low-confidence ASR
    'o se amo', 'um saúde', 'uma saúde'
  ];
  ...
  try {
    ...
    activeSession = await ai.live.connect({
      ...
      config: {
        ...
        systemInstruction: `
          Role: High-Fidelity Portuguese Transcriber.
          Context: Live political debate.
          
          Guidelines:
          1. TRANSCRIBE ONLY SPOKEN WORDS. Ignore all background noise, applause, or music.
          2. FILTER HALLUCINATIONS: Do not output "Obrigado", "Legendas", "Sous-titres" unless clearly spoken in context.
          3. DEALING WITH OVERLAP: Focus on the dominant voice.
          4. PUNCTUATION: Use natural punctuation to delineate sentences.
          5. NO META-COMMENTARY: Never explain what you are doing. Just output the text.
          6. CONTINUITY: Do not break sentences excessively.
          7. IMPORTANT: NEVER STOP LISTENING. IGNORE SILENCE.
        `,
      },
      callbacks: {
        onopen: () => {
        ...
        onmessage: (msg: LiveServerMessage) => {
           ...
           if (msg.serverContent?.turnComplete) {
               ...
               // Logic: Commit if decent length OR sentence end. 
               // If it's a tiny fragment without punctuation, hold it (it might be the start of next sentence).
               if (trimmed.length > 30 || (trimmed.length > 5 && isSentenceEnd)) {
                   commitBuffer();
               } else {
                   // Keep waiting, relying on silence watchdog to flush if no more audio comes.
                   scheduleSilenceCommit();
               }
           }
        },
